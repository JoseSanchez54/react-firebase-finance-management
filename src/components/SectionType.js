...
export default function SectionType({ type }) {
  ...
  useEffect(() => {
    const query = `/users/${auth.currentUser.email}/${type}`;
    onValue(ref(db, query), (snapshot) => {
      ...
      if (snapval !== null) {
        ...
        const allMonthsByYear = Object.entries(snapval).map(
          ([key, dbMonth]) => ({ [key]: Object.keys(dbMonth) })
        );
        if (
          !allMonthsByYear.some(
            (year) => Object.keys(year)[0] === currentDate.year.toString()
          )
        ) {
          allMonthsByYear.push({ [currentDate.year.toString()]: [] });
        }

        const availableMonths = allMonthsByYear.find(
          (year) => Object.keys(year).toString() === date.year.toString()
        )[date.year];
  ...
  return (
    <section>
      ...
      <Form type={type} handleChangeDate={handleChangeDate} />
