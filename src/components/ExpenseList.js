  ...
  useEffect(() => {
    ...
    onValue(ref(db, query), (snapshot) => {
      ...
      if (snapval !== null) {
        let dbData =
          monthActive === "all"
            ? Object.values(snapval).flatMap((dbMonth) =>
                Object.values(dbMonth)
              )
            : Object.values(snapval);

        setData(dbData);

        let total = dbData.reduce(
          (tot, current) => tot + parseInt(current.price),
          0
        );
        setTotExpense(total);
      }
      ...
      {data.length !== 0 ? (
        <>
          {data.map((expense) => (
            <li
              key={expense.id}
              className={editing && expense.id === editData.id ? "visible" : ""}
            >
                ...
                <>
                  ...
                  {btn && (
                    <div className="commands">
                      ...
                      >
                        <span id={expense.id}>Editar</span>
                      </button>
                    </div>
                  )}
                </>
