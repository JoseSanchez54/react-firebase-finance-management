...
export default function SectionSummary() {
  ...
  const handleClick = (e) => {
    ...
    if (typesSelected.includes(type)) {
      setTypesSelected(
        typesSelected.filter((typeSelected) => typeSelected !== type)
      );
    } else {
  ...
  useEffect(() => {
    ...
    for (const type of types) {
      ...
      if (
        userData[type] &&
        userData[type][date.year] &&
        userData[type][date.year][date.month]
      ) {
        for (const expence in userData[type][date.year][date.month]) {
          totalType += parseInt(
            userData[type][date.year][date.month][expence].price
          );
        }
      }
