...
export default function Form({ type, handleChangeDate }) {
  ...
  const addUserExpense = (e) => {
    ...
    set(
      ref(
        db,
        `/users/${auth.currentUser.uid}/${type}/${yearExpense}/${monthExpense}/${idExpense}/`
      ),
      { ...trimmedExpense, id: idExpense, mail: auth.currentUser.email }
    )
      .then(() =>
        console.log(
          `${type !== "earnings" ? "Expense" : "Earning"} saved successfully!`
        )
      )
